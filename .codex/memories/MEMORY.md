...
applies_to: WSL2 testing for `/home/lh/code/sno-cli`; read `ai-docs/testing/wsl2-test-method.md` before creating, starting, testing, stopping, or deleting Azure resources.

## Reusable knowledge

- The complete account identifiers, VM configuration, commands, user-context rule, proof, failures, security handling, current resource state, and cleanup commands are in `ai-docs/testing/wsl2-test-method.md`.
...
- The last run is fully cleaned up: all three Azure test resource groups were deleted. Start every new run by locating an allowed region again.
