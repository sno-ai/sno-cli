...
Shell and PowerShell installers are first executed against the exact staged archives by overriding their supported download base URL to the local artifact directory. The workflow then uploads the same bytes to a mutable draft release, downloads them back from GitHub on every native platform, and repeats installer execution. Only that verified draft can be published and frozen as the immutable release. The workflow finally repeats installation from the anonymous public URLs; no identity receipt is issued until all three stages pass. A later failure is corrected only by a new patch release; immutable assets are never replaced.

### Publish a synchronized patch release from a semantic version tag

Version `0.1.0` is already immutable on crates.io and cannot be republished. The production-distribution work therefore increments the crate to `0.1.1`, reviews and publishes that exact crate, then creates tag `v0.1.1` from the same source commit. The release workflow accepts `v<crate-version>` tags only. GitHub Releases become public distribution only after the repository is public.
...
5. Make the repository public and enable immutable releases. Record `GET /repos/sno-ai/sno-cli` with `.visibility == "public"` and `GET /repos/sno-ai/sno-cli/immutable-releases` with `.enabled == true`, using GitHub API version `2026-03-10`.
6. From a clean checkout of the reviewed commit, package and publish crate `0.1.1`; download the registry archive and require its SHA-256 to match the local package archive.
7. Create tag `v0.1.1` at that exact reviewed commit. The tag-triggered workflow repeats the public/immutable API predicates, tests every extracted archive and staged installer, downloads and retests a GitHub draft, publishes that same draft as immutable, and repeats both installers from their anonymous public URLs.
