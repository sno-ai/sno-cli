...
Version `0.1.0` is already immutable on crates.io and cannot be republished. Version `0.1.1` failed closed before builds because a standard workflow token cannot read the administration-only immutable-release setting. Version `0.1.2` failed during cross-platform cargo-dist bootstrap. Version `0.1.3` failed during strict checksum parsing. Version `0.1.4` failed because GitHub CLI cannot download an unpublished draft by tag. Version `0.1.5` carried the numeric draft ID but lacked push access. Version `0.1.6` fixed access and proved both Linux draft installers; its macOS jobs failed because Bash 3.2 lacks `mapfile`, while Windows retained a carriage return in the resource ID. Cleanup deleted every draft. All tags remain fixed at their reviewed commits. The forward-only recovery is `0.1.7`: draft selection uses one strict JSON scalar, removes Windows carriage returns, and validates the resulting numeric ID without `mapfile`.
