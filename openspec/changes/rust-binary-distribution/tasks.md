...
- [ ] 4.6 Preserve the failed `v0.1.1` through `v0.1.4` attempts without moving their tags, publish the forward-only `0.1.5` crate from the repaired reviewed commit, then verify all seven archives, installers, checksums, manifests, and available attestations from the immutable `v0.1.5` GitHub Release.
