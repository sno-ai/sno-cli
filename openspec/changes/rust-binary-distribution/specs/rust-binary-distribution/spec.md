...
The production binary release SHALL use forward-only version `0.1.7` after versions `0.1.1` through `0.1.6` failed closed at successive release gates. The release workflow SHALL accept only semantic version tags matching the crate version and SHALL use the committed lockfile for all builds. The crates.io package and GitHub tag SHALL identify the same reviewed source version. Only the nested draft-asset verification chain SHALL receive `contents: write`; staged, candidate, and public verification SHALL remain read-only. Draft asset selection SHALL run on Bash 3.2 and normalize Windows line endings before validating numeric IDs. The existing `v0.1.1` through `v0.1.6` tags MUST NOT be moved.
