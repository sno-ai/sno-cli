...
- Give both exit-code consumers, `run_rem.sh` and `run_rem_noop.sh`, an enumerated routing table and fail closed on unknown codes; keep `run_rem.sh` routing independent of human-readable messages and reserve 20 and 21 for that runner's own failures.
- Record the raw state, outcome class, and exit code in the existing CLI and `run_rem.sh` traces for the unrecognised-state path.
...
- Does not change which operation names either runner accepts. It does change the rejected-operation exit number inside `run_rem.sh`'s validation block; name ownership and exit ownership are separate lines.
