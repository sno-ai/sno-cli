...
run_negative_control() {
	bash "$BASE_RUNNER" negative-qcg12
}
...
verify_green_receipt
output_file="$(mktemp /tmp/rem-qcg12-baseline.XXXXXX)"
negative_file="$(mktemp /tmp/rem-qcg12-negative.XXXXXX)"
cleanup() {
	rm -f -- "$output_file" "$negative_file"
}
...
rg -Fx 'Section5 boundary_reached observations=24/24' "$output_file" >/dev/null || \
...
if [[ "$negative_status" -ne 1 ]]; then
	printf 'negative-qcg12 returned %s, expected 1\n' "$negative_status" >&2
	exit 1
fi
rg '^NEGATIVE CONTROL RED QCG12:' "$negative_file" >/dev/null

printf 'reviewed_plan_sha256=%s\n' "$REVIEWED_PLAN_SHA256"
printf 'proof_execution_count=24\n'
printf 'proof_marker=qcg12_dual_runner_real_boundary_routing_table\n'
