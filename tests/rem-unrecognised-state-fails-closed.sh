...
run_negative_control() {
	bash "$BASE_RUNNER" negative-qcg13
}
...
verify_green_receipt
output_file="$(mktemp /tmp/rem-qcg13-baseline.XXXXXX)"
negative_file="$(mktemp /tmp/rem-qcg13-negative.XXXXXX)"
...
if [[ "$negative_status" -ne 1 ]]; then
	printf 'negative-qcg13 returned %s, expected 1\n' "$negative_status" >&2
	exit 1
fi
rg '^NEGATIVE CONTROL RED QCG13:' "$negative_file" >/dev/null

printf 'reviewed_plan_sha256=%s\n' "$REVIEWED_PLAN_SHA256"
printf 'proof_execution_count=24\n'
printf 'proof_marker=qcg13_unrecognised_state_fails_closed\n'
