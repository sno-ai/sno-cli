...
run_negative_control() { bash "$BASE_RUNNER" negative-qcg17; }
...
verify_green_receipt
output_file="$(mktemp /tmp/rem-qcg17-baseline.XXXXXX)"
negative_file="$(mktemp /tmp/rem-qcg17-negative.XXXXXX)"
...
rg '^progress 1/24 case=section-first-code-0 exit=0 ' "$output_file" >/dev/null
rg '^progress 12/24 case=sibling-first-code-0 exit=0 ' "$output_file" >/dev/null
...
if [[ "$negative_status" -ne 1 ]]; then
	printf 'negative-qcg17 returned %s, expected 1\n' "$negative_status" >&2
	exit 1
fi
rg '^NEGATIVE CONTROL RED QCG17:' "$negative_file" >/dev/null

printf 'reviewed_plan_sha256=%s\n' "$REVIEWED_PLAN_SHA256"
printf 'proof_execution_count=24\n'
printf 'proof_marker=qcg17_independent_landing_orders_one_store_two_jobs\n'
