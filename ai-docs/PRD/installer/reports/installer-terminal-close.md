...
- **QCG-2 now passes.** The real Reach candidate, real utility archives and gate-produced four-unit skills passed the complete local staged-consumer verifier on gpt1: 157 assertions, no failures. [The staged close](installer-real-staged-close.md) records the exact artifacts, 30 mapped files/stamps, declaration/gate parity, numeric/program/capability rules, shared/custom roots, absolute installed commands, both utility doctor/repair checks and whole-tree negative snapshots. The canonical proof tool updated this row. Historical missing-input and negative-only reports describe earlier states.
...
The current delivery lint rejects only QCG-9; see [current gate output](installer-real-staged-delivery-gate.log). The done command is attempted again with this truthful blocked report; its refusal is retained in installer-real-staged-done-gate.log. The project is not marked done. This is the fixture-reachable execution result requested in the dispatch, not final acceptance or publication.
