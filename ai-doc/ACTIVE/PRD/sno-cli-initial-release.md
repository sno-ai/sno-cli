...
- Current release state on 2026-07-15 PDT: crates.io packages `sno` `0.1.0` through `0.1.7` are public under `SnoInfo`; GitHub repository `sno-ai/sno-cli` is public with immutable releases enabled; immutable GitHub Release `v0.1.7` publishes seven verified target archives plus Shell and PowerShell installers. Versions `0.1.1` through `0.1.6` remain registry-only after their workflows failed closed at successive gates and deleted any mutable draft. Every protected version tag remains fixed.
